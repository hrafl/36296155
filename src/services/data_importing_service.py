...
import pandas as pd
from src.navigation.pages.products.products_page_content_reader import ProductsPageContentReader
...
def read_products(material_type: str, product_type: str, characteristic: str):
    ...
    while True:
        ...
        browser.get(url)
        products_page_content_reader = ProductsPageContentReader(browser)
        products = products_page_content_reader.get_products()
        ...
        if not products_page_content_reader.is_there_next_page():
            break
        else:
            counter += 1
